      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
          ...
          <p style={{ fontSize: '1rem', color: muted, maxWidth: 480, margin: '0 auto', lineHeight: 1.8 }}>
            Preencha os dados. Os números atualizam em tempo real com benchmarks do mercado imobiliário de SP.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '1.5rem', alignItems: 'start' }}>
          ...
          <div style={{ borderRadius: 16, padding: '2rem', background: panel, border: `1px solid ${border}` }}>
            ...
            <OptionPills label="Perfil da operação" sub="Ajusta os benchmarks de investimento"
              options={[{ value: 'corretor', label: 'Corretor Autônomo' }, { value: 'imobiliaria', label: 'Imobiliária Média' }, { value: 'grande', label: 'Grande Porte' }]}
              value={perfil} onChange={setPerfil} />

            <div style={{ borderTop: `1px solid ${border}`, margin: '1.5rem 0' }} />

            <Slider label="Leads adormecidos na base" sub="Contatos que chegaram e nunca converteram"
              value={baseLeads} min={200} max={15000} step={100} onChange={setBaseLeads}
              format={v => v >= 1000 ? `${(v / 1000).toFixed(1)}k` : `${v}`} />
            <Slider label="Novos leads por mês"
            ...
            <Slider label="Número de corretores / agentes"
              value={corretores} min={1} max={40} step={1} onChange={setCorretores}
              format={v => `${v} pessoa${v > 1 ? 's' : ''}`} accent="#6ab4ff" />

            <div style={{ borderTop: `1px solid ${border}`, margin: '1.5rem 0' }} />

            <Slider label="Comissão média por venda fechada" sub="Valor que entra por negócio concluído"
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <p style={{ fontSize: '0.875rem', color: '#D8D4CC', margin: 0 }}>
                    Você perde <strong style={{ color: isHighLoss ? '#E05555' : gold }}>{Math.round(perdaPct * 100)}%</strong> dos leads por lentidão
                  </p>
                  <p style={{ fontSize: '0.75rem', color: muted, margin: '4px 0 0' }}>≈ {leadsPerdidos} leads/mês indo para o concorrente</p>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              {[
                { label: 'Receita na base adormecida', icon: '💤', value: receitaReat, sub: `${leadsReat} leads reativáveis (2,5%)`, accent: gold },
            ...
            <p style={{ fontSize: '0.7rem', textAlign: 'center', color: '#2A2A3A', lineHeight: 1.6, margin: 0 }}>
              Benchmarks: Relatório Imobiliário SP 2025–2026 · Febraban 2025. Projeções conservadoras.
            </p>
          </div>
        </div>
      </div>
