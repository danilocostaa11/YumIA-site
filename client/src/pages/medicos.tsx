...
export default function Medicos() {
  // SEO meta
  useEffect(() => {
    document.title = 'YumIA para Clínicas — IA que Recupera Receita e Reduz No-Show'
    let desc = document.querySelector('meta[name=description]')
    if (desc) desc.setAttribute('content', 'IA para clínicas e consultórios: agendamento inteligente, redução de no-show, reativação de pacientes e dashboard por médico. Método MAPA com ROI mensurável.')
    let canon = document.querySelector('link[rel=canonical]')
    if (canon) canon.setAttribute('href', 'https://www.yumia.com.br/medicos')
  }, [])
      ...
      <section className="hero-section" style={{ background: C.bg }}>
        ...
        <div className="hero-inner">
          <div>
            ...
            <h1 className="hero-h1" style={{ color: C.white }} aria-label="Sua clínica deixa pacientes na mesa.">
            ...
            <form className="contact-form" style={{ background: C.panel, border: `1px solid ${C.border}` }} onSubmit={handleSubmit}>
              ...
              <div className="form-row">
                {[
                  ...
                  { name: 'empresa' as const, label: 'Clínica / Consultório', placeholder: 'Nome da clínica' },
                ].map(f => (
              ...
              <div className="form-row">
                {[
                  ...
                  { name: 'email' as const, label: 'E-mail', placeholder: 'voce@clinica.com.br' },
                ].map(f => (
                  <div key={f.name} className="form-field">
                    <label className="form-label" style={{ color: C.muted }}>{f.label}</label>
                    <input
