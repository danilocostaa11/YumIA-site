...
export default function Imoveis() {
  // SEO meta
  useEffect(() => {
    document.title = 'YumIA para Imobiliárias — IA que Recupera Receita Perdida'
    let desc = document.querySelector('meta[name=description]')
    if (desc) desc.setAttribute('content', 'IA para imobiliárias: atendimento 24/7, automação de funil, reativação de base e dashboard de gestão. Método MAPA com ROI mensurável.')
    let canon = document.querySelector('link[rel=canonical]')
    if (canon) canon.setAttribute('href', 'https://www.yumia.com.br/imoveis')
  }, [])
            ...
            <form className="contact-form" style={{ background: C.panel, border: `1px solid ${C.border}` }} onSubmit={handleSubmit}>
              ...
              <div className="form-row">
                {[
                  ...
                  { name: 'empresa' as const, label: 'Imobiliária', placeholder: 'Nome da imobiliária' },
                ].map(f => (
              ...
              <div className="form-row">
                {[
                  ...
                  { name: 'email' as const, label: 'E-mail', placeholder: 'voce@imobiliaria.com.br' },
                ].map(f => (
                  <div key={f.name} className="form-field">
                    <label className="form-label" style={{ color: C.muted }}>{f.label}</label>
                    <input
