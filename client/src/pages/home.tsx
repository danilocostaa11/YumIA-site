...
export default function Home() {
  // SEO meta
  useEffect(() => {
    document.title = 'YumIA — Infraestrutura de Crescimento com IA'
    let desc = document.querySelector('meta[name=description]')
    if (desc) desc.setAttribute('content', 'A YumIA instala sites, automações, atendimento com IA e dashboards para eliminar vazamentos comerciais e transformar operação em crescimento com controle.')
    let canon = document.querySelector('link[rel=canonical]')
    if (canon) canon.setAttribute('href', 'https://www.yumia.com.br/')
  }, [])
          ...
          {submitted ? (
            ...
            <form className="contact-form" style={{ background: C.panel, border: `1px solid ${C.border}` }} onSubmit={handleSubmit}>
              ...
              <div className="form-row">
                {[
                  { name: 'nome' as const, label: 'Nome completo', placeholder: 'Seu nome' },
                  { name: 'empresa' as const, label: 'Empresa / Segmento', placeholder: 'Empresa e seu setor' },
                ].map(f => (
              ...
              <div className="form-row">
                {[
                  { name: 'whatsapp' as const, label: 'WhatsApp', placeholder: '+55 (11) 96418-0674' },
                  { name: 'email' as const, label: 'E-mail', placeholder: 'voce@empresa.com.br' },
                ].map(f => (
                  <div key={f.name} className="form-field">
                    <label className="form-label" style={{ color: C.muted }}>{f.label}</label>
                    <input
